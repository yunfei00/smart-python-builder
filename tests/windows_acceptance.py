"""Maintained Windows release acceptance matrix against real generated executables."""
...
BASE=ROOT/'workspace'/('acceptance-'+uuid.uuid4().hex)
...
def record(case,result,notes):
    ...
    (BASE/'acceptance-results.json').write_text(json.dumps(records,indent=2,ensure_ascii=False),encoding='utf-8')
...
for name,code in [
    ('Tkinter',"import tkinter as tk\nr=tk.Tk();tk.Label(r,text='Smart Python Builder').pack();r.mainloop()"),
    ('PySide6',"from PySide6.QtWidgets import QApplication,QLabel\na=QApplication([]);w=QLabel('Smart Python Builder');w.show();a.exec()"),
...
source=project('failure',"raise RuntimeError('intentional-acceptance-failure')")
fake=FakeNotifier()

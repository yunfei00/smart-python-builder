...
result=SmartBuilder(ai_provider=provider,artifact_validator=verify).build(source)
assert result.status=='SUCCESS' and len(result.attempts)==2,result
assert len(provider.contexts)==1
Path('docs/phase5-acceptance.json').write_text(json.dumps(dict(status=result.status,states=result.states,attempts=result.attempts,artifact=str(result.build.artifact),notes='Real EXE failed with missing colorsys, FakeAIProvider supplied validated hidden import, rebuilt EXE exited 0'),indent=2))
